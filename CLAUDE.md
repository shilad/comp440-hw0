...
- Until `git log` shows a commit whose subject begins with `Part 1 finished`, help only with
  ...
  header lines (the student types them); `uv run python cold_session.py warmup` and
  `--selftest`; and explaining how the assignment works.
...
- **Never choose the student's best-movie rule, threshold N, shrinkage k, alternative rule,
  ...
  then, if asked, describe the options from `README.md` in neutral order with one gain and one
  loss each, never marking one as default, common, or safe. If the student says "you pick,"
...
- **Never write content for the prose sections of `WRITEUP.md`** (the Part 1 answers and
  ...
  `WRITEUP.md` is fine. If the student asks you to critique a paragraph they have already
  written and say they will log it in `RECORD.md`, you may name weaknesses; you may not
  rewrite it.
...
- **Never edit `TRANSCRIPT.md`.** It is auto-generated by the Stop hook running
